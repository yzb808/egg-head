	...
	<div class="g-bd" id="content">
		...
		<div class="g-mn g-mn-1 <#if (RequestParameters.show)??>lt0<#else>lt24</#if>">
			...
			<div class="g-cnt g-cnt-2">
				<div class="m-tab">
				    <ul id="tab">
				        <li class="z-act"><a href="javascript:void(0);">单机状态</a></li>
				        <li><a href="/sentry/status/cluster?clusterId=${RequestParameters.clusterId}&status=all">集群状态</a></li>
				        <li><a href="/sentry/status/vip?clusterId=${RequestParameters.clusterId}&status=all">VIP状态</a></li>
				    </ul>
			     </div>
				<form id="search-form">
					...
					<div class="m-form2">
						<div class="fmitem">
							<label class="fmlab">主机：</label>
							<div class="fmcnt">
								<select class="u-ipt" name="hostId">
								<#if serverList??>
								    <option value="">全部</option>
									<#list serverList as server>
									 <option value="${server.serverId}" <#if RequestParameters.hostId?? && RequestParameters.hostId == '${server.serverId}' >selected="selected"</#if>><#if server.hostAlias?? && server.hostAlias != ''>${server.hostAlias}<#else>${server.hostName}</#if></option>
							        </#list>
						        </#if>
								</select>
							</div>
					...
					<tbody>
						<#if statuses??>
							...
							<tr>
								...
								<td title="<#if x.hostAlias?? && x.hostAlias != ''>${x.hostAlias}<#else>${x.hostName}</#if>">
									<#if x.hostAlias?? && x.hostAlias != ''>${x.hostAlias}<#else>${x.hostName}</#if>
								</td>
								<td>
									<span>${x.monitorItemName!}</span>
									<a href="/sentry/monitor/mod?clusterId=${RequestParameters.clusterId}&monitorItemId=${x.monitorItemId!}" class="u-tag-monitor"></a>
...
<textarea name="jst" id="form-rule">
<div>
	<ul class="f-cb js-options" id="js-options">
        <li data-rangetype="0"{if rangeType=="1h"} class="cur"{/if}>最近1小时</li>
        <li data-rangetype="1"{if rangeType=="3h"} class="cur"{/if}>最近3小时</li>
        <li data-rangetype="2"{if rangeType=="6h"} class="cur"{/if}>最近6小时</li>
        <li data-rangetype="3"{if rangeType=="today"} class="cur"{/if}>今天</li>
        <li data-rangetype="4"{if rangeType=="yesterday"} class="cur"{/if}>昨天</li>
        <li data-rangetype="5"{if rangeType=="3d"} class="cur"{/if}>最近3天</li>
        <li data-rangetype="8" class="{if rangeType=="selfdefine"}cur{/if} j-custom"><a>自定义>></a></li>
        {if viewType == "single"}
        <li class="j-instance"><a>多实例比较&gt;&gt;</a></li>
        {/if}
    </ul>
    <div class="customtime j-time" id="js-time" {if rangeType!="selfdefine"}style="display:none"{/if}>
        <input type="text" onClick="WdatePicker({dateFmt:\'yyyy-MM-dd HH:mm:ss\',autoPickDate:true})" value="${startTime}" class="Wdate u-ipt u-ipt-s1" name="startTime"/> ~ 
        <input type="text" onClick="WdatePicker({dateFmt:\'yyyy-MM-dd HH:mm:ss\',autoPickDate:true})" value="${endTime}" class="Wdate u-ipt u-ipt-s1" name="endTime"/>
        <a class="u-btn u-btn-c1 j-confirm">确定</a>
    </div>
</div>
<table class="m-table" id="list">
	<thead>
		<tr>
			<th>规则名称</th>
			<th>状态</th>
...
<script src="${jsPages}/sentry/status/list.js"></script>
</body>
</html>
</#escape>
